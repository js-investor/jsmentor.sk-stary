...
/** Udalosť v čase: od začiatku daného roka iný mesačný vklad, alebo jednorazový vklad na začiatku roka. */
export type InvEvent = { id: string; kind: "monthly" | "lump"; year: number; amount: number };
const MAX_EVENTS = 6;
...
type ScenarioResult = {
  ...
  hasCosts: boolean;
  hasEvents: boolean;
  /** dopad udalostí na výslednú hodnotu (po dani) oproti rovnakému portfóliu bez udalostí */
  eventsDelta: number;
  /** o koľko viac (menej) sa kvôli udalostiam vloží */
  eventsInvested: number;
  /** indexy rokov (0 = začiatok) so značkou udalosti v grafe */
  eventIdx: number[];
};
...
export function mountInvesticnaCalculator(): () => void {
  ...
  function sanitizeData(data: Partial<InvestFormData> | InvestFormData | null | undefined): InvestFormData {
    ...
    const duration = Math.max(1, Math.min(50, Math.round(n(data?.duration, 20))));
    return {
      ...
      events: (Array.isArray(data?.events) ? data!.events : [])
        .map((e, i) => ({
          ...
          year: Math.max(1, Math.min(duration, Math.round(n(e?.year, 1)))),
          amount: Math.max(0, Math.min(10000000, n(e?.amount, 0))),
        }))
        .slice(0, MAX_EVENTS),
    };
  }
  ...
  type SimResult = {
    final: number;
    series: number[];
    /** kumulatívne vložené na konci rokov 0..duration */
    invested: number[];
    totalInvested: number;
    entryPaid: number;
    annualPaid: number;
   ...
   */
  function simulate(d: InvestFormData, fees: FeeSet, withEvents = true): SimResult {
    ...
    const changes = active.filter((e) => e.kind === "monthly").sort((x, y) => x.year - y.year);
    const lumps = new Map<number, number>();
    for (const e of active) if (e.kind === "lump") { const m = (e.year - 1) * 12 + 1; lumps.set(m, (lumps.get(m) ?? 0) + e.amount); }
    ...
    for (let m = 1; m <= d.duration * 12; m++) {
      // zmena pravidelného vkladu od prvého mesiaca zvoleného roka
      for (const c of changes) if ((c.year - 1) * 12 + 1 === m) monthly = c.amount;
  ...
  function calculateScenario(data: InvestFormData | null | undefined): ScenarioResult {
    ...
    // rovnaké poplatky bez udalostí → dopad udalostí
    const base = simulate(d, { entry: d.entryFee, annual: d.annualFee, perf: d.perfFee }, false);
    const taxOf = (final: number, invested: number) => (d.tax && final - invested > 0 ? (final - invested) * TAX_RATE : 0);

    const totalInvested = net.totalInvested;
    const finalBeforeTax = net.final;
    const tax = taxOf(finalBeforeTax, totalInvested);
    const finalValue = finalBeforeTax - tax;
    const baseFinal = base.final - taxOf(base.final, base.totalInvested);
    ...
    const labels: string[] = [];
    for (let i = 0; i <= d.duration; i++) labels.push("Rok " + i);
    ...
    return {
      ...
      eventIdx: activeEvents.map((e) => e.year - 1),
    };
  }
  ...
  /** Koncový bod série a roky s udalosťou zvýrazníme bodkou. */
  const eventIdx = new Set<number>();
  const lastPointRadius = (ctx: ScriptableContext<"line">) =>
    ctx.dataIndex === ctx.dataset.data.length - 1 || eventIdx.has(ctx.dataIndex) ? 5 : 0;
  ...
  function updateChart(s: ScenarioResult | null): void {
    ...
    eventIdx.clear();
    s.eventIdx.forEach((i) => eventIdx.add(i));
    if (chartInstance) {
  ...
  function updateUi(s: ScenarioResult): void {
    ...
    updateFeesUi(s);
    show("events-chip", s.hasEvents);
    setText("eventsDeltaVal", (s.eventsDelta >= 0 ? "+" : "−") + formatCurrency(Math.abs(s.eventsDelta)));
    setText("eventsInvestedVal", (s.eventsInvested >= 0 ? "+" : "−") + formatCurrency(Math.abs(s.eventsInvested)));
    show("legend-events", s.hasEvents);
    refreshEventHints();
  ...
  /* ---------------- Udalosti v čase ---------------- */
  const eventsList = document.getElementById("inv-events-list");
  const addEventBtn = document.getElementById("inv-add-event");
  const eventsToggle = document.getElementById("inv-events-toggle");
  const eventsContent = document.getElementById("inv-events-content");
  const eventsArrow = document.getElementById("inv-events-arrow");
  let eventCounter = 0;
  const durationNow = () => Math.max(1, Math.min(50, Math.round(parseFloat((document.getElementById("inv-duration") as HTMLInputElement | null)?.value || "20") || 20)));
  ...
  const eventHint = (e: InvEvent) =>
    e.amount > 0
      ? e.kind === "lump"
        ? `V roku ${START_YEAR + e.year - 1} vložíš jednorazovo ${formatCurrency(e.amount)}.`
        : `Od roku ${START_YEAR + e.year - 1} investuješ ${formatCurrency(e.amount)} mesačne${e.amount !== monthlyNow() ? ` namiesto ${formatCurrency(monthlyNow())}` : ""}.`
      : "Zadaj sumu, inak sa udalosť nepočíta.";
  function renderEvents(): void {
    if (!eventsList) return;
    const duration = durationNow();
    events.forEach((e) => { e.year = Math.max(1, Math.min(duration, e.year)); });
    eventsList.innerHTML = events
      .map((e, i) => {
        const years = Array.from({ length: duration }, (_, k) => `<option value="${k + 1}"${e.year === k + 1 ? " selected" : ""}>${k + 1}. rok</option>`).join("");
        return `<div class="inv-event" data-id="${esc(e.id)}">
      ...
      .join("");
    if (addEventBtn) addEventBtn.style.display = events.length >= MAX_EVENTS ? "none" : "";
  }
  function refreshEventHints(): void {
    eventsList?.querySelectorAll<HTMLElement>(".inv-event").forEach((row) => {
      const e = events.find((x) => x.id === row.dataset.id);
      const hint = row.querySelector<HTMLElement>("[data-hint]");
      if (e && hint) hint.textContent = eventHint(e);
    });
  }
  const eventsListHandler = (ev: Event) => {
    const target = ev.target as HTMLElement | null;
    const row = target?.closest<HTMLElement>(".inv-event");
    if (!row) return;
    const e = events.find((x) => x.id === row.dataset.id);
    if (!e) return;
    if (ev.type === "click") {
    ...
    const field = (target as HTMLInputElement | HTMLSelectElement | null)?.dataset?.field;
    if (!field) return;
    if (field === "kind") {
      e.kind = (target as HTMLSelectElement).value === "lump" ? "lump" : "monthly";
      renderEvents();
      inv_calculate();
      return;
    }
    if (field === "year") e.year = Math.max(1, Math.round(Number((target as HTMLSelectElement).value) || 1));
    if (field === "amount") e.amount = Math.max(0, Number((target as HTMLInputElement).value) || 0);
    inv_calculate();
  };
  eventsList?.addEventListener("input", eventsListHandler);
  eventsList?.addEventListener("change", eventsListHandler);
  eventsList?.addEventListener("click", eventsListHandler);
  ...
  const eventsToggleHandler = () => {
    if (!eventsContent || !eventsArrow) return;
    const isHidden = window.getComputedStyle(eventsContent).display === "none";
    eventsContent.style.display = isHidden ? "block" : "none";
    eventsArrow.style.transform = isHidden ? "rotate(180deg)" : "rotate(0deg)";
    eventsToggle?.setAttribute("aria-expanded", String(isHidden));
  };
  eventsToggle?.addEventListener("click", eventsToggleHandler);
  renderEvents();
  ...
  inputIds.forEach((id) => {
    ...
    const fn = () => {
      if (id === "inv-duration") renderEvents();
      inv_calculate();
    };
    if (el) {
  ...
  return () => {
    ...
    eventsList?.removeEventListener("input", eventsListHandler);
    eventsList?.removeEventListener("change", eventsListHandler);
    eventsList?.removeEventListener("click", eventsListHandler);
    addEventBtn?.removeEventListener("click", addEventHandler);
    eventsToggle?.removeEventListener("click", eventsToggleHandler);
    chartInstance?.destroy();
    chartInstance = null;
